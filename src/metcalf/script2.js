...
function saveBookedSlot(day, timeSlot, name){
    let bookedSlots2 = JSON.parse(localStorage.getItem('bookedSlots2')) || {};
    if(!bookedSlots2[day]){
        bookedSlots2[day] = [];
    }
    bookedSlots2[day].push({ time: timeSlot, bookedBy: name });
    localStorage.setItem('bookedSlots2', JSON.stringify(bookedSlots2));
}
//gets the slots that are booked
function getBookedSlots(){
    return JSON.parse(localStorage.getItem('bookedSlots2')) || {};
}
...
function generateEmptySlots() {
    ...
    if (currentDayOfWeek === 0 && currentHour === 0 && minutes === 0 && seconds === 1) {
        localStorage.removeItem('bookedSlots2');
    }

    const bookedSlots2 = getBookedSlots();
    ...
    days.forEach((column, index) => {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            if (bookedSlots2[dayName]) {
                const bookedSlot2 = bookedSlots2[dayName].find(slot => slot.time === i);
                if (bookedSlot2) {
                    emptyBlock.classList.add('booked');
                    emptyBlock.setAttribute('title', `Booked by: ${bookedSlot2.bookedBy}`);
                }
            }
