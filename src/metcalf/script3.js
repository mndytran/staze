...
function saveBookedSlot(day, timeSlot, name){
    let bookedSlots3 = JSON.parse(localStorage.getItem('bookedSlots3')) || {};
    if(!bookedSlots3[day]){
        bookedSlots3[day] = [];
    }
    bookedSlots3[day].push({ time: timeSlot, bookedBy: name });
    localStorage.setItem('bookedSlots3', JSON.stringify(bookedSlots3));
}
//gets the slots that are booked
function getBookedSlots(){
    return JSON.parse(localStorage.getItem('bookedSlots3')) || {};
}
...
function generateEmptySlots() {
    ...
    if (currentDayOfWeek === 0 && currentHour === 0 && minutes === 0 && seconds === 1) {
        localStorage.removeItem('bookedSlots3');
    }
    const bookedSlots3 = getBookedSlots();
    ...
    days.forEach((column, index) => {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            if (bookedSlots3[dayName]) {
                const bookedSlot3 = bookedSlots3[dayName].find(slot => slot.time === i);
                if (bookedSlot3) {
                    emptyBlock.classList.add('booked');
                    emptyBlock.setAttribute('title', `Booked by: ${bookedSlot3.bookedBy}`);
                }
            }
