...
function saveBookedSlot(day, timeSlot, name){
    let bookedSlots7 = JSON.parse(localStorage.getItem('bookedSlots7')) || {};
    if(!bookedSlots7[day]){
        bookedSlots7[day] = [];
    }
    bookedSlots7[day].push({ time: timeSlot, bookedBy: name });
    localStorage.setItem('bookedSlots7', JSON.stringify(bookedSlots7));
}
//gets the slots that are booked
function getBookedSlots(){
    return JSON.parse(localStorage.getItem('bookedSlots7')) || {};
}
...
function generateEmptySlots() {
    ...
    if (currentDayOfWeek === 0 && currentHour === 0 && minutes === 0 && seconds === 1) {
        localStorage.removeItem('bookedSlots7');
    }

    const bookedSlots7 = getBookedSlots();
    ...
    days.forEach((column, index) => {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            if (bookedSlots7[dayName]) {
                const bookedSlot7 = bookedSlots7[dayName].find(slot => slot.time === i);
                if (bookedSlot7) {
                    emptyBlock.classList.add('booked');
                    emptyBlock.setAttribute('title', `Booked by: ${bookedSlot7.bookedBy}`);
                }
            }
