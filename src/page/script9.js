...
function saveBookedSlot(day, timeSlot, name){
    let bookedSlots9 = JSON.parse(localStorage.getItem('bookedSlots9')) || {};
    if(!bookedSlots9[day]){
        bookedSlots9[day] = [];
    }
    bookedSlots9[day].push({ time: timeSlot, bookedBy: name });
    localStorage.setItem('bookedSlots9', JSON.stringify(bookedSlots9));
}
//gets the slots that are booked
function getBookedSlots(){
    return JSON.parse(localStorage.getItem('bookedSlots9')) || {};
}
...
function generateEmptySlots() {
    ...
    if (currentDayOfWeek === 0 && currentHour === 0 && minutes === 0 && seconds === 1) {
        localStorage.removeItem('bookedSlots9');
    }

    const bookedSlots9 = getBookedSlots();
    ...
    days.forEach((column, index) => {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            if (bookedSlots9[dayName]) {
                const bookedSlot9 = bookedSlots9[dayName].find(slot => slot.time === i);
                if (bookedSlot9) {
                    emptyBlock.classList.add('booked');
                    emptyBlock.setAttribute('title', `Booked by: ${bookedSlot9.bookedBy}`);
                }
            }
