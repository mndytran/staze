...
function saveBookedSlot(day, timeSlot, name){
    let bookedSlots4 = JSON.parse(localStorage.getItem('bookedSlots4')) || {};
    if(!bookedSlots4[day]){
        bookedSlots4[day] = [];
    }
    bookedSlots4[day].push({ time: timeSlot, bookedBy: name });
    localStorage.setItem('bookedSlots4', JSON.stringify(bookedSlots4));
}
//gets the slots that are booked
function getBookedSlots(){
    return JSON.parse(localStorage.getItem('bookedSlots4')) || {};
}
...
function generateEmptySlots() {
    ...
    if (currentDayOfWeek === 0 && currentHour === 0 && minutes === 0 && seconds === 1) {
        localStorage.removeItem('bookedSlots4');
    }

    const bookedSlots4 = getBookedSlots();
    ...
    days.forEach((column, index) => {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            if (bookedSlots4[dayName]) {
                const bookedSlot4 = bookedSlots4[dayName].find(slot => slot.time === i);
                if (bookedSlot4) {
                    emptyBlock.classList.add('booked');
                    emptyBlock.setAttribute('title', `Booked by: ${bookedSlot4.bookedBy}`);
                }
            }
