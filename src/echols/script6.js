...
function saveBookedSlot(day, timeSlot, name){
    let bookedSlots6 = JSON.parse(localStorage.getItem('bookedSlots6')) || {};
    if(!bookedSlots6[day]){
        bookedSlots6[day] = [];
    }
    bookedSlots2[day].push({ time: timeSlot, bookedBy: name });
    localStorage.setItem('bookedSlots6', JSON.stringify(bookedSlots6));
}
//gets the slots that are booked
function getBookedSlots(){
    return JSON.parse(localStorage.getItem('bookedSlots6')) || {};
}
...
function generateEmptySlots() {
    ...
    if (currentDayOfWeek === 0 && currentHour === 0 && minutes === 0 && seconds === 1) {
        localStorage.removeItem('bookedSlots6');
    }

    const bookedSlots6 = getBookedSlots();
    ...
    days.forEach((column, index) => {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            if (bookedSlots6[dayName]) {
                const bookedSlot6 = bookedSlots6[dayName].find(slot => slot.time === i);
                if (bookedSlot6) {
                    emptyBlock.classList.add('booked');
                    emptyBlock.setAttribute('title', `Booked by: ${bookedSlot6.bookedBy}`);
                }
            }
