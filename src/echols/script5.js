...
function saveBookedSlot(day, timeSlot, name){
    let bookedSlots5 = JSON.parse(localStorage.getItem('bookedSlots5')) || {};
    if(!bookedSlots5[day]){
        bookedSlots5[day] = [];
    }
    bookedSlots5[day].push({ time: timeSlot, bookedBy: name });
    localStorage.setItem('bookedSlots5', JSON.stringify(bookedSlots5));
}
//gets the slots that are booked
function getBookedSlots(){
    return JSON.parse(localStorage.getItem('bookedSlots5')) || {};
}
...
function generateEmptySlots() {
    ...
    if (currentDayOfWeek === 0 && currentHour === 0 && minutes === 0 && seconds === 1) {
        localStorage.removeItem('bookedSlots5');
    }

    const bookedSlots5 = getBookedSlots();
    ...
    days.forEach((column, index) => {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            if (bookedSlots5[dayName]) {
                const bookedSlot5 = bookedSlots5[dayName].find(slot => slot.time === i);
                if (bookedSlot5) {
                    emptyBlock.classList.add('booked');
                    emptyBlock.setAttribute('title', `Booked by: ${bookedSlot5.bookedBy}`);
                }
            }
